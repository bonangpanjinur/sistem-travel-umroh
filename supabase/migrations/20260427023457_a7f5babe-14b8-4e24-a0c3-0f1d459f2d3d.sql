-- =====================================================
-- Sistem Hak Akses 2 Lapis: Role Default + User Override
-- =====================================================

-- 1. Tabel role_permissions
...
-- 2. Replace check_user_permission dengan logika 2 lapis
...
BEGIN
  ...
  -- 3. Role-level: ALLOW if any of user's roles enables this permission
  SELECT EXISTS (
    SELECT 1
...
-- 3. RPC: get all effective permission keys for a user
...
BEGIN
  ...
  combined AS (
    -- role-allowed minus user-disabled
    ...
    UNION
    -- user-enabled overrides (additional grants)
...
-- 4. Seed permissions_list dari menu registry (idempotent)
...
-- 5. Seed menu_items dari registry (idempotent)
...
-- 6. Seed default role_permissions (idempotent)
-- Owner: semua kecuali "roles" (manajemen role hanya super_admin) — owner tetap dapat untuk fleksibilitas
...
-- Branch Manager: hampir semua kecuali pengelolaan role/users sistem
...
-- Operational: fokus operasional keberangkatan
INSERT INTO public.role_permissions (role, permission_key, is_enabled) VALUES
