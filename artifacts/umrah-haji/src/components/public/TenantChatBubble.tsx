...
const FAQ: Record<string, string> = {
  ...
  "harga": "💰 Harga paket Umroh & Haji bervariasi tergantung fasilitas dan musim. Lihat [daftar paket lengkap](/packages) di halaman Paket kami atau hubungi kami untuk penawaran terbaik.",
...
function formatContent(text: string, linkColor = "#7c3aed") {
  return text
    .replace(/\*\*(.*?)\*\*/g, "<strong>$1</strong>")
    .replace(/\*(.*?)\*/g, "<em>$1</em>")
    .replace(
      /\[([^\]]+)\]\((\/[^)]*)\)/g,
      `<a href="$2" onclick="event.stopPropagation();window.location.href='$2';return false;" style="color:${linkColor};text-decoration:underline;font-weight:500;cursor:pointer;">$1</a>`
    )
    .replace(
      /\[([^\]]+)\]\((https?:\/\/[^)]*)\)/g,
      `<a href="$2" target="_blank" rel="noopener noreferrer" style="color:${linkColor};text-decoration:underline;font-weight:500;">$1</a>`
    )
    .split("\n").join("<br/>");
}
