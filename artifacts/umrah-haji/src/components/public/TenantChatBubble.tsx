...
interface TenantChatBubbleProps {
  ...
  /** Warna aksen utama bubble (tailwind class, e.g. "from-emerald-600 to-teal-700") */
  gradientFrom?: string;
  gradientTo?: string;
}
...
export function TenantChatBubble({
  ...
  const gradient  = `bg-gradient-to-r ${gradientFrom} ${gradientTo}`;
  const btnGrad   = `bg-gradient-to-br ${gradientFrom} ${gradientTo}`;
      ...
      {!open && (
        <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-2">
          ...
          >
            ...
            {showPulse && (
              <span className="absolute inset-0 rounded-full bg-teal-400 opacity-40 animate-ping" />
            )}
          </button>
        </div>
      )}
              ...
              <div className="flex-1 overflow-y-auto p-3 space-y-3 bg-gray-50">
                {messages.map(msg => (
                  <div key={msg.id} className={`flex gap-2 ${msg.role === "user" ? "flex-row-reverse" : ""}`}>
                    ...
                    <div className={`max-w-[82%] rounded-2xl px-3 py-2 text-sm leading-relaxed shadow-sm ${msg.role === "user" ? "bg-emerald-600 text-white rounded-tr-sm" : "bg-white border rounded-tl-sm text-gray-800"}`}>
                      <div
              ...
              <div className="px-3 pt-2 pb-1 flex gap-1.5 overflow-x-auto scrollbar-hide bg-white border-t border-gray-50">
                {SUGGESTED.map(q => (
                  <button
                    ...
                    disabled={loading}
                    className="shrink-0 text-[11px] bg-emerald-50 text-emerald-700 px-2.5 py-1 rounded-full border border-emerald-200 hover:bg-emerald-100 transition-colors whitespace-nowrap disabled:opacity-50"
                  >
                    {q}
                  </button>
                ))}
              ...
              <div className="px-3 pb-3 pt-2 bg-white space-y-2">
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 text-sm border border-gray-200 rounded-full px-4 py-2 outline-none focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 transition-all bg-gray-50"
                  />
                  <button
                    onClick={() => sendMessage()}
                    disabled={loading || !input.trim()}
                    className="w-9 h-9 rounded-full bg-emerald-600 hover:bg-emerald-700 disabled:opacity-40 flex items-center justify-center shrink-0 transition-colors"
                  >
                    {loading
