...
import { Radio, BookOpen, ChevronRight, User } from "lucide-react";
...
export function TourLeaderOffView({ ctx }: Props) {
  const today = new Date();
  const todayLabel = format(today, "EEEE, d MMMM yyyy", { locale: id });
  const firstName = ctx.profile?.full_name?.split(" ")[0] ?? "Tour Leader";
    ...
    <JamaahAppShell>
        ...
        <div className="flex items-center justify-between">
          <div>
            <div className="flex items-center gap-1.5 mb-0.5">
              ...
              <p className="text-[11px] opacity-75 uppercase tracking-wider">Tour Leader · {greeting},</p>
            </div>
      ...
      <div className="px-4 py-4 space-y-4 pb-32">
        ...
        <div className="rounded-2xl border bg-card p-4 space-y-2">
          <p className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Aktivitas Tour Leader</p>
          {[
            { to: "/tour-leader/dashboard", icon: "🎯", label: "Dashboard Tour Leader", desc: "Lihat jadwal dan riwayat keberangkatan" },
            ...
            { to: "/jamaah/itinerary", icon: "📅", label: "Itinerary Template", desc: "Kelola template itinerary keberangkatan" },
          ].map(item => (
            <Link
              key={item.to}
              to={item.to}
