...
import {
  ...
  CheckCircle, Clock, XCircle, ReceiptText, Upload, Loader2,
  Image as ImageIcon, Star, MessageSquare, Send, ThumbsUp,
} from "lucide-react";
  ...
  return (
    <div className="flex items-center gap-0.5">
      {[1, 2, 3, 4, 5].map((s) => (
        <Star
          key={s}
          className={cn(starSize, s <= rating ? "fill-amber-400 text-amber-400" : "fill-muted text-muted-foreground/30")}
        />
      ))}
    </div>
  );
...
function ProductReviewRow({
  ...
  item: { id: string; product_id: string; product_name: string; product_image: string | null; quantity: number };
  orderId: string;
  userId: string;
  ...
  const handleSubmit = () => {
    if (!rating) return;
    submit.mutate(
      { orderId, productId: item.product_id, userId, customerId, rating, comment },
      ...
      {submitted && existingReview ? (
        <div className="bg-green-50 border border-green-200 rounded-xl p-4 space-y-2">
          <div className="flex items-center gap-2">
            ...
            <p className="text-sm font-semibold text-green-800">Ulasan terkirim</p>
          </div>
          <StarDisplay rating={existingReview.rating} size="md" />
          <p className="text-xs text-green-700 font-medium">{RATING_LABEL[existingReview.rating]}</p>
        ...
        <div className="bg-green-50 border border-green-200 rounded-xl p-4 space-y-2">
          <div className="flex items-center gap-2">
            <ThumbsUp className="h-4 w-4 text-green-600" />
            <p className="text-sm font-semibold text-green-800">Ulasan terkirim! Terima kasih.</p>
          </div>
          <StarDisplay rating={rating} size="md" />
          <p className="text-xs text-green-700 font-medium">{RATING_LABEL[rating]}</p>
        ...
        <div className="space-y-3 pl-1">
          <div className="space-y-1">
            <p className="text-xs text-muted-foreground font-medium">Rating produk ini:</p>
            <StarRatingInput value={rating} onChange={setRating} size="md" />
            {rating > 0 && (
              <p className="text-xs font-semibold text-amber-600">{RATING_LABEL[rating]}</p>
            )}
          </div>

          <div className="space-y-1">
            <p className="text-xs text-muted-foreground font-medium">Komentar (opsional):</p>
            <Textarea
          ...
          >
            {submit.isPending ? (
              ...
              <><Send className="h-4 w-4 mr-1.5" />Kirim Ulasan</>
            )}
          </Button>
        </div>
      )}
...
export default function StoreOrderDetail() {
  ...
  const { data: myReviews = [] } = useMyOrderReviews(
    order?.status === "delivered" ? orderId : undefined
  );
  ...
  const canReview = order.status === "delivered" && !!user;

  // Map existing reviews by product_id for quick lookup
  const reviewByProductId = Object.fromEntries(
    myReviews.map((r) => [r.product_id, r])
  );
  const allReviewed = canReview && order.items && order.items.length > 0 &&
  ...
  return (
    <DynamicPublicLayout>
      ...
      <div className="max-w-2xl mx-auto px-4 py-6 space-y-4">
        ...
        {/* ═══ REVIEW SECTION ═══ */}
        {canReview && order.items && order.items.length > 0 && (
          <Card className="border-amber-200 bg-amber-50/40">
            <CardHeader className="pb-3">
              <CardTitle className="text-base flex items-center gap-2 text-amber-800">
                <Star className="h-4 w-4 fill-amber-400 text-amber-400" />
                {allReviewed ? "Ulasan Kamu" : "Beri Ulasan Produk"}
              </CardTitle>
              {!allReviewed && (
                <p className="text-xs text-amber-700">
                  Pesanan sudah diterima! Bagikan pengalamanmu untuk membantu jamaah lain.
                </p>
              )}
            </CardHeader>
              ...
              {order.items.map((item, idx) => (
                <div key={item.id}>
                  ...
                  <ProductReviewRow
                    ...
                    existingReview={reviewByProductId[item.product_id] ?? null}
                  />
                </div>
              ))}
              ...
              {allReviewed && (
                <div className="pt-1 text-center">
                  <p className="text-xs text-amber-700 font-medium flex items-center justify-center gap-1.5">
                    ...
                    Semua produk sudah kamu ulas. Terima kasih!
                  </p>
                </div>
              )}
            </CardContent>
            ...
            {order.items?.map((item) => (
              <div key={item.id} className="flex gap-3">
                ...
                <div className="flex-1">
                  ...
                  {/* Show existing review snippet under item */}
                  {reviewByProductId[item.product_id] && (
                    <div className="flex items-center gap-1.5 mt-1">
                      <StarDisplay rating={reviewByProductId[item.product_id].rating} />
                      <span className="text-xs text-amber-600 font-medium">
                        {RATING_LABEL[reviewByProductId[item.product_id].rating]}
                      </span>
                    </div>
                  )}
                </div>
        ...
        {/* CTA banner saat belum review */}
        {canReview && !allReviewed && (
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-start gap-3">
            <MessageSquare className="h-5 w-5 text-amber-600 mt-0.5 shrink-0" />
            <div>
              <p className="text-sm font-semibold text-amber-800">Bagikan pengalamanmu!</p>
              <p className="text-xs text-amber-700 mt-0.5">
                Ulasanmu sangat berarti bagi jamaah lain. Scroll ke atas untuk memberikan rating.
              </p>
            </div>
          </div>
        )}
