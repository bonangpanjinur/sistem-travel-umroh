...
/**
 * Canonical menu registry — kept aligned with `menu_items` table in DB.
 * Used as fallback when DB query fails and as source for command palette.
 * 
 * NOTE: The DB is now the source of truth. This array is only used as a
 * safety fallback; do NOT push it via auto-sync at login.
 */
...
/**
 * Manual menu sync (admin-only). Uses `text` parameter signature matching
 * the active backend `bulk_sync_menu_items(_menu_items text)`.
