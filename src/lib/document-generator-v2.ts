...
// Helper to add letterhead with dynamic settings
function addLetterhead(
  doc: jsPDF,
  company: CompanyInfo = defaultCompanyInfo,
  settings: DocumentSettings = defaultDocumentSettings
) {
  ...
  return y + 10; // Return starting Y position for content
}

// Helper to add footer with dynamic settings
function addFooter(
  doc: jsPDF,
  pageNum: number,
  totalPages: number,
  ...
  if (settings.document_footer_show_timestamp) {
    doc.text(
  ...
  if (settings.document_footer_show_page_number) {
    doc.text(
...
export function generateLeaveLetter(
  ...
  const doc = new jsPDF();
  let y = addLetterhead(doc, company, settings);
...
export function generatePassportLetter(
  ...
  doc.setFontSize(14);
  doc.setFont('helvetica', 'bold');
  doc.text('SURAT PERMOHONAN PEMBUATAN PASPOR', pageWidth / 2, y, { align: 'center' });
  y += 15;
...
Dengan ini mengajukan permohonan untuk pembuatan Paspor Biasa (Paspor Umum) dengan tujuan perjalanan untuk ${data.purpose}.

Demikian surat permohonan ini saya ajukan. Atas perhatian dan kerjasamanya, saya ucapkan terima kasih.`;
  ...
  y += 25;
  doc.setFont('helvetica', 'bold');
  doc.text(data.customerName, pageWidth - 60, y);
...
export function generateInvoice(
  ...
  y += 5;
  doc.text(`Tanggal: ${format(data.invoiceDate, 'd MMMM yyyy', { locale: id })}`, 14, y);
  y += 5;
  doc.text(`Jatuh Tempo: ${format(data.dueDate, 'd MMMM yyyy', { locale: id })}`, 14, y);
  y += 10;
  ...
  y += 5;
  doc.text(`Telp: ${data.customer.phone}`, 14, y);
  if (data.customer.email) {
    y += 5;
    doc.text(`Email: ${data.customer.email}`, 14, y);
  }
  y += 10;
  ...
  // Totals
  doc.setFont('helvetica', 'normal');
  doc.text(`Subtotal: ${formatCurrency(data.subtotal)}`, pageWidth - 60, y);
  y += 6;

  if (data.discount) {
    doc.text(`Diskon: -${formatCurrency(data.discount)}`, pageWidth - 60, y);
    y += 6;
  }

  if (data.tax) {
    doc.text(`Pajak: ${formatCurrency(data.tax)}`, pageWidth - 60, y);
    y += 6;
  }

  doc.setFont('helvetica', 'bold');
  doc.setFontSize(12);
  doc.text(`Total: ${formatCurrency(data.total)}`, pageWidth - 60, y);

  y += 15;

  // Bank info
  if (settings.invoice_show_bank_info && data.bankInfo) {
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(10);
  ...
  // Notes
  if (settings.invoice_show_notes_section && data.notes) {
    y += 15;
    doc.setFont('helvetica', 'bold');
...
export function generateETicket(
  ...
  const doc = new jsPDF();
  const pageWidth = doc.internal.pageSize.width;
  const [r, g, b] = hexToRgb(settings.eticket_header_color);
...
export function generateUmrahCertificate(
  ...
  const pageWidth = doc.internal.pageSize.width;
  const pageHeight = doc.internal.pageSize.height;
  const [br, bg, bb] = hexToRgb(settings.certificate_border_color);
  const [tr, tg, tb] = hexToRgb(settings.certificate_text_color);
