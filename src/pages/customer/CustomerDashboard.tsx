      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="space-y-6">
          ...
          {savings && savings.length > 0 && (
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-base flex items-center gap-2"><PiggyBank className="h-4 w-4 text-primary" /> Tabungan Aktif</CardTitle>
              </CardHeader>
              <CardContent className="space-y-3">
                {savings.map((s: any) => (
                  <div key={s.id} className="p-2 bg-muted/50 rounded">
                    <p className="text-sm font-medium">{s.package?.name}</p>
                    <p className="text-sm text-muted-foreground">{formatCurrency(s.paid_amount || 0)} / {formatCurrency(s.target_amount || 0)}</p>
                  </div>
                ))}
                <Button variant="link" size="sm" className="px-0" asChild><Link to="/customer/my-savings">Kelola Tabungan →</Link></Button>
              </CardContent>
            </Card>
          )}
